...
CREATE TABLE business_lifetime (
    business_id VARCHAR(30) PRIMARY KEY,
    years_active INT,
    avg_reviews_per_year NUMERIC(10,2),
    FOREIGN KEY (business_id) REFERENCES business(business_id) ON DELETE CASCADE
);

INSERT INTO business_lifetime (business_id, years_active, avg_reviews_per_year)
SELECT 
    business_id,
    EXTRACT(YEAR FROM MAX(date)) - EXTRACT(YEAR FROM MIN(date)) + 1 AS years_active,
    COUNT(*) * 1.0 / (EXTRACT(YEAR FROM MAX(date)) - EXTRACT(YEAR FROM MIN(date)) + 1) AS avg_reviews_per_year
FROM review
