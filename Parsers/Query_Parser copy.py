...
def parseUserData():
    print("Parsing users and inserting into DB...")
            ...
            try:
                ...
                cursor.execute("""
                    INSERT INTO yelp_user (
                        user_id, name, yelping_since, fans, review_count,
                        average_stars, useful, funny, cool
                    ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
                """, (user_id, name, yelping_since, fans, review_count, avg_stars, useful, funny, cool))
                count_line += 1
                if count_line % 1000 == 0:
                    conn.commit()
            except Exception as e:
                print(f"Error inserting user {data.get('user_id')}: {e}")
                traceback.print_exc()
                conn.rollback()
    conn.commit()
    print(f"Inserted {count_line} users.")
